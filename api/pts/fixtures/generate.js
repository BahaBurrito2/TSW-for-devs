...
export default async function (req, res) {
  ...
  if (!league || !["D1","D2"].includes(league.division_code)) return res.status(400).json({ error:"Choose a PTS Division 1 or Division 2 league." });
  const { rows: teams } = await db.query("SELECT team_id FROM league_teams WHERE league_id = $1 ORDER BY team_id",[league_id]);
  if (teams.length !== 8) return res.status(400).json({ error:"Each PTS division needs exactly 8 clubs before its 7-gameweek fixture list can be generated." });
